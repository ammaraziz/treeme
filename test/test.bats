...
@test "Test: color taxa - Success" {
    run treeme.R -t $TREE -m $META -o "${OUTPUT_BASE}/colortaxa.pdf" -c $COLOR_VAR
...
@test "Test: color taxa - Failure" {
    run treeme.R -t $TREE -m $META -o $OUTPUT_FAILED -c "NotReal"
...
@test "Test: tippoint - Success" {
    run treeme.R -t $TREE -m $META -o $OUTPUT_BASE -s $SHAPE_VAR
...
@test "Test: tippoint missing shape-var - Failure" {
    run treeme.R -t $TREE -m $META -o $OUTPUT_FAILED -s "NotReal"
...
@test "Test: basic tree with no labels, has colors and shapes - Success" {
    run treeme.R -t $TREE -m $META -o "${OUTPUT_BASE}/shapecolor_notext.pdf" \
